...
class BaseDetector():
    ...
    def predict_array(self, anArray, model, need_rot=False, test_shape=None, lefttop=(0,0), classes=None,
        ...
        ret = self.run(im_scale, model, thresh=thresh, classes=classes)
        ...
        return out
    def uncrop_predict(self, predict, im_size, bbox):
        h, w = im_size
        left, top, right, bottom = bbox

        if 'boxes' in predict:
            predict['boxes'][:, [0,2]] += left
            predict['boxes'][:, [1,3]] += top

        if 'masks' in predict:
            predict['masks'] = [cv2.copyMakeBorder(m.astype('uint8'), top, h-bottom,left,w-right, borderType=cv2.BORDER_CONSTANT, value=0)
                                for m in predict['masks']]

        if 'kepoints' in predict:
            predict['kepoints'][:, 0] += left
            predict['kepoints'][:, 1] += top
