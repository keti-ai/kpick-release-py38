...
def get_default_hybird_grasp_cfg():
    args = get_base_cfg()
    args.merge_with(get_default_suction_args())
    return args


class HybridGraspDetector(GripDetector, SuctionDetector):
    ...
    def detect_hybrid_grasp(self, rgbd, grip_net_args, suction_net_args):
        timer = Timer()
        ...
        if len(locs)==0:
            ...
            return None
        Grip = Grip[locs, :]
        print(f'Detected {len(Grip)} hybrid grasps')
        timer.pin_time('Get_hybrid_grasp')
        ...
        print(f'Best grip: {np.round(best_grip, decimals=3).tolist()}')
        print(timer.pin_times_str())

        return ret

def get_hybrid_grasp_detector_obj(Detector=HybridGraspDetector):
    from ketisdk.vision.base.base_objects import BasObj
    class HybridGraspDetectorObj(Detector, BasObj):
        def __init__(self, args=None, cfg_path=None, name='unnamed', default_args=None):
            BasObj.__init__(self, args=args, cfg_path=cfg_path, name=name, default_args=get_default_hybird_grasp_cfg())
        ...
        def detect_and_show_hybrid(self, rgbd, disp_mode='rgb'):
            ret = Detector.detect_hybrid_grasp(self, rgbd=rgbd, grip_net_args=self.args.grip_net,
    ...
    return HybridGraspDetectorObj


def get_hybrid_grasp_detector_gui(DetectorObj=get_hybrid_grasp_detector_obj()):
    from kpick.base.base import DetGui
    class HybridGraspDetectorGuiObj(DetectorObj, DetGui):

        def gui_process_single(self, rgbd, method_ind=0, filename='unnamed', disp_mode='rgb', detected=None, **kwargs):
            if method_ind == 0:
                ret = DetectorObj.detect_and_show_hybrid(self, rgbd=rgbd, disp_mode=disp_mode)
            if method_ind == 1:
    ...
    return HybridGraspDetectorGuiObj


def demo_hybrid_grasp_gui(title='Hyubrid Grasp Detection GUI', Detector=HybridGraspDetector,
                          cfg_path='configs/hybrid_grasp.cfg', default_cfg_path='configs/hybrid_grasp_default.cfg',
    ...
    detect_module = GuiModule(
        get_hybrid_grasp_detector_gui(DetectorObj=get_hybrid_grasp_detector_obj(Detector=Detector)),
        type='hybrid_grasp_detector', name='Hybrid Grasp Detector',
